...
pub fn load_lsp_config(root: &Utf8Path) -> std::result::Result<Config, ConfigError> {
    ...
    let mut config = if config_path.exists() {
        match read_file_to_string_with_limit(&config_path, MAX_CONFIG_BYTES)? {
            LimitedFileRead::Text(contents) => Config::load_from_str(&contents)?,
            LimitedFileRead::TooLarge => {
...
pub(super) fn read_file_bytes_with_limit(path: &Utf8Path, limit: u64) -> std::io::Result<Vec<u8>> {
    crate::file_read::read_regular_file_bytes_with_limit(
        path.as_std_path(),
        limit,
